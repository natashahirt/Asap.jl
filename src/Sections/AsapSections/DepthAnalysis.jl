...
Find the intersection point (if it exists) between two lines:
...
returns a boolean and a vector: 
 ...
 - (false, nothing) if an intersection point does not exist on L1
"""
...
function sutherland_hodgman_abs(section::PolygonalSection, y::Float64; return_section = false)
    ...
    if y > section.ymax
        new_polygon = [[0., 0.], [0., 0.]]
    #if absolute position encompasses the whole section, return section
