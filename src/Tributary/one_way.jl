...
function get_tributary_polygons_one_way(
    ...
    length(t_vals) < 2 && return [_make_tributary_with_depths(i, NTuple{2,Float64}[], pts_orig, 0.0, 0.0) for i in 1:m]
    ...
    for i in 1:m
        ...
        push!(results, _make_tributary_with_depths(i, poly_xy, pts_orig, area, frac))
    end
