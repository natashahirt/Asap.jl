...
# Run: julia --project=. external/Asap/test/tributary/debug_tributaries.jl
# =============================================================================
...
function get_abs_vertices(trib::Asap.TributaryPolygon, orig_pts::Vector{NTuple{2,Float64}})
    ...
    return Asap.vertices(trib, beam_start, beam_end)
end

"""Get absolute vertices from parametric tributary given Meshes.Point vertices."""
function get_abs_vertices(trib::Asap.TributaryPolygon, verts::Vector{<:Meshes.Point})
    pts = [(Float64(Meshes.coords(v).x.val), Float64(Meshes.coords(v).y.val)) for v in verts]
