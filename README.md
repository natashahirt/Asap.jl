...
## Creating Shells from Custom Meshes

For shells with custom geometry (vaults, domes, imported meshes), Asap provides flexible APIs that accept meshes in various formats.

### Quick Reference

| Input Format | Function Signature |
|--------------|-------------------|
| Raw points + triangles | `Shell(points, triangles, section, supports)` |
| Meshes.jl SimpleMesh | `Shell(mesh, section, supports)` |
| DelaunayTriangulation | `Shell(tri, section, supports)` |

All three share the same support options and produce the same shell elements.

### 1. Raw Points + Triangles (Most Flexible)

Create shells from any point/connectivity data—from files, procedural generation, etc.

```julia
# Define vertex coordinates (unitless = meters, or use Unitful)
points = [
    (0.0, 0.0, 0.0),   # vertex 1
    (1.0, 0.0, 0.0),   # vertex 2
    (0.5, 1.0, 0.2),   # vertex 3
    (1.5, 1.0, 0.2)    # vertex 4
]

# Define triangle connectivity (1-based indices)
triangles = [(1, 2, 3), (2, 4, 3)]

section = ShellSection(0.15u"m", 30u"GPa", 0.2)

# Create shells with automatic support detection
shells = Shell(points, triangles, section)           # pins all at z≈0
shells = Shell(points, triangles, section, [1, 2])   # explicit supports
shells = Shell(points, triangles, section, :none)    # all free
```

**Supported point formats:**
- `Vector{NTuple{3,T}}` - tuples: `[(0,0,0), (1,0,0), ...]`
- `Vector{Vector{T}}` - vectors: `[[0,0,0], [1,0,0], ...]`
- `Matrix{T}` - N×3 matrix: `[0 0 0; 1 0 0; ...]`
- Coordinates can be `Float64` (meters) or `Quantity` (any length unit)

**Supported triangle formats:**
- `Vector{NTuple{3,Int}}` - tuples: `[(1,2,3), (2,4,3), ...]`
- `Vector{Vector{Int}}` - vectors: `[[1,2,3], [2,4,3], ...]`

### 2. Meshes.jl SimpleMesh

For integration with the Julia geometry ecosystem (GeoIO, MeshIO, etc.):

```julia
using Meshes

# Create mesh from points + connectivity
pts = [Point(0.0, 0.0, 0.0), Point(1.0, 0.0, 0.0), ...]
conn = [connect((1, 2, 3)), connect((2, 4, 3)), ...]
mesh = SimpleMesh(pts, conn)

# Or load from file via GeoIO
# mesh = GeoIO.load("model.msh")

shells = Shell(mesh, section)              # auto-pin at z≈0
shells = Shell(mesh, section, :xz_plane)   # pin at y≈0
shells = Shell(mesh, section, indices)     # explicit support indices
```

### 3. DelaunayTriangulation

For meshes generated by Asap's `mesh()` function or DelaunayTriangulation.jl:

```julia
# Generate 2D triangulation from corner nodes
n1, n2, n3, n4 = [Node([x*u"m", y*u"m", 0u"m"], :free) for (x,y) in corners]
tri = mesh((n1, n2, n3, n4), 6)  # refinement level 6

shells = Shell(tri, section, [1, 2, 3, 4])  # pin corners
shells = Shell(tri, section, :xy_plane)     # pin all (z=0 mesh)
```

### Support Options

All mesh-based `Shell()` functions accept a `supports` argument:

| Value | Description | Pinned Nodes |
|-------|-------------|--------------|
| `:xy_plane` | Vertices where z ≈ 0 | Default for flat meshes |
| `:xz_plane` | Vertices where y ≈ 0 | Side-supported shells |
| `:yz_plane` | Vertices where x ≈ 0 | Front-supported shells |
| `:none` | All nodes free | For manual BC application |
| `Vector{Int}` | Explicit indices | `[1, 2, 5, 6]` |

**Tolerance:** Use `tol` keyword to adjust plane detection (default: 0.01 meters)

```julia
shells = Shell(mesh, section, :xy_plane; tol=0.001)  # tighter tolerance
```

### Manual Boundary Condition Application

For complex BCs (symmetry, mixed conditions), start with `:none` and apply BCs using node positions:

```julia
# Create shells with all nodes free
shells = Shell(points, triangles, section, :none)
nodes = get_nodes(shells)

# Apply BCs based on geometry
for node in nodes
    x = ustrip(u"m", node.position[1])
    y = ustrip(u"m", node.position[2])
    z = ustrip(u"m", node.position[3])
    
    # Rigid diaphragm at y=0: fix x, z, θy (DOFs 1, 3, 5)
    if abs(y) < tol
        node.dof[[1, 3, 5]] .= false
    end
    
    # Symmetry at y=L/2: fix y, θx, θz (DOFs 2, 4, 6)
    if abs(y - L/2) < tol
    ...
    if abs(x) < tol
        node.dof[[1, 2, 3]] .= false  # fix translations only
    end
end
...
**DOF mapping:** `[x, y, z, θx, θy, θz]` where `false` = fixed, `true` = free

**Common BC patterns:**

| Condition | Fixed DOFs | Code |
|-----------|-----------|------|
| Pinned (translations fixed) | x, y, z | `node.dof[[1,2,3]] .= false` |
| Fixed (clamped) | All 6 | `node.dof .= false` |
| Roller (Z only) | z | `node.dof[3] = false` |
| Symmetry (normal) | normal translation + tangent rotations | varies |
| Rigid diaphragm | varies | see example above |

### Complete Example: Barrel Vault Shell

```julia
using Asap

# Generate barrel vault mesh (parametric cylinder)
R, L = 25.0, 50.0   # radius, length
θ_max = 40 * π/180  # arc angle
n = 8               # mesh density

# Create points on cylindrical surface
points = NTuple{3,Float64}[]
for j in 0:n, i in 0:n
    θ = i/n * θ_max
    y = j/n * L/2
    push!(points, (R*sin(θ), y, R*(cos(θ)-1)))
end
...
for j in 1:n, i in 1:n
    idx(i,j) = (j-1)*(n+1) + i
    push!(triangles, (idx(i,j), idx(i+1,j), idx(i+1,j+1)))
    push!(triangles, (idx(i,j), idx(i+1,j+1), idx(i,j+1)))
end

# Create shells with all nodes free initially
section = ShellSection(0.25u"m", 30u"GPa", 0.2)
shells = Shell(points, triangles, section, :none)
nodes = get_nodes(shells)

# Apply Scordelis-Lo benchmark BCs
tol = R/n/100
for node in nodes
    x = ustrip(u"m", node.position[1])
    y = ustrip(u"m", node.position[2])
...
# Load and solve
load = AreaLoad(shells, [0, 0, -90]u"Pa")  # Vector form!
model = ShellModel(nodes, shells, [load])
solve!(model)
```
...
load = AreaLoad(shells, 1.2u"kPa"; direction=(1.0, 0.0, 0.0))
```

### Vector Form (Intuitive)

Specify load as a 3D vector `[px, py, pz]` combining magnitude and direction:

```julia
# 90 Pa downward (gravity)
load = AreaLoad(shells, [0, 0, -90]u"Pa")

# 50 Pa at 45° in XZ plane
load = AreaLoad(shells, [35.36, 0, -35.36]u"Pa")

# Horizontal wind + vertical gravity combined
load = AreaLoad(shells, [500, 0, -2500]u"Pa")

# Using N/m² (same as Pa)
load = AreaLoad(shells, [0, 0, -5000]u"N/m^2")
```

The vector is decomposed internally: magnitude = `norm([px,py,pz])`, direction = normalized vector.

### Tributary Distribution

For distributing shell loads to supporting beams:
